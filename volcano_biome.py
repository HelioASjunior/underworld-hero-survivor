"""
volcano_biome.py — Chão e decorações do bioma Volcano.

Assets: assets/backgrounds/inferno/
  tiles.png          → 46 tiles 32×32 (probabilidades do inferno_tiles.json)
  lavanova.png       → poça de lava animada (grid 6×4 de 256×256, usa 6 frames)
  mountain_big.png   → 3 rochas grandes (strip 64×64)
  mountain_medium.png→ 3 rochas médias  (strip 48×48)
  mountain_small.png → 4 rochas pequenas(strip 32×32)
  mountain_tall.png  → 1 rocha alta     (48×64)
  mountain_wide.png  → 1 rocha larga    (64×48)
  geyser.png         → geyser           (24×24)
...
_INFERNO = os.path.join("backgrounds", "inferno")

# ---------------------------------------------------------------------------
# Probabilidades dos tiles (inferno_tiles.json)
# ---------------------------------------------------------------------------
_TILE_PROBS = [
    0.60, 0.40, 0.60, 6.00, 1.10, 4.50, 1.50, 1.50, 4.50, 4.50,
    0.70, 0.80, 0.90, 0.90, 1.10, 5.10, 1.10, 1.50, 0.90, 0.80,
    0.80, 0.50, 0.70, 0.50, 3.50, 1.10, 0.70, 0.80, 0.80, 0.40,
    3.50, 3.60, 1.10, 0.90, 2.50, 0.90, 1.60, 0.60, 0.40, 0.40,
    0.30, 0.25, 1.20, 1.20, 0.30, 0.30,
]
_TILE_COUNT  = len(_TILE_PROBS)   # 46
_TILE_SIZE   = 32
_GROUND_SIZE = 256
...
def build_volcano_ground(loader):
    """Surface 256×256 composta por tiles do inferno/tiles.png."""
    frames = loader.load_spritesheet(
        os.path.join(_INFERNO, "tiles"),
...
# ---------------------------------------------------------------------------
# Configs bioma-específicas (lavanova + rochas)
# ---------------------------------------------------------------------------
_BIOME_DECO_CFGS = [
    # ── Lava nova animada ────────────────────────────────────────────────
    ...
    dict(kind="geyser", grid=1100, prob=0.18, spd=0.0, size=(96, 96),
         rel="geyser.png", fw=24, fh=24, collide=False),
    # ── Rochas / montanhas ───────────────────────────────────────────────
...
def _load_strip(path, fw, fh, display_size, frame_count=None):
    ...
    try:
        ...
        print(f"[VolcanoBiome] erro {path}: {e}")
        return []
...
class VolcanoDecoManager:
    """
    Decorações do bioma Volcano.
    Combina decorações específicas do vulcão (lavanova, rochas) com os
    ...
    def __init__(self, asset_dir):
        ...
        self._active:   dict[tuple, _VolcanoDeco] = {}
        self._all_cfgs: list[dict]                = []

    def load_frames(self):
        inferno_dir = os.path.join(self._asset_dir, "backgrounds", "inferno")
        chao_dir    = os.path.join(self._asset_dir, "ui", "chao")

        # ── Decorações específicas do vulcão ────────────────────────────────
        for cfg in _BIOME_DECO_CFGS:
            kind = cfg["kind"]
            path = os.path.join(inferno_dir, cfg["rel"])
            fc   = cfg.get("frame_count")
            frames = _load_strip(path, cfg["fw"], cfg["fh"], cfg["size"], fc)
            self._frames[kind] = frames
            print(f"[VolcanoBiome] {kind}: {len(frames)} frames" if frames else f"[VolcanoBiome] {kind}: FALHOU")
    ...
    def _cell_rng(self, gx, gy, kind):
        return random.Random((gx * 61348271) ^ (gy * 38291047) ^ hash(kind))
    ...
    def _cell_start_frame(self, gx, gy, kind, n):
        rng = random.Random((gx * 55291847) ^ (gy * 72834901) ^ hash(kind) ^ 0xBEEF)
